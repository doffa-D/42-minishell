...
int if_builtins_in_parent(t_all *my_struct, int c_of_s)
{
...
int if_builtins_in_chiled(t_all *my_struct, int c_of_s)
{
	if(my_struct->each_cmd[c_of_s].cmd[0] && ft_strlen(my_struct->each_cmd[c_of_s].cmd[0]) \
	 &&!ft_strncmp(my_struct->each_cmd[c_of_s].cmd[0], "cd", ft_strlen(my_struct->each_cmd[c_of_s].cmd[0])))
		return (1);
...
int builtins_in_parent(t_all *my_struct, int c_of_s)
{
	int i = 0;
	if(my_struct->each_cmd[c_of_s].cmd[0] && ft_strlen(my_struct->each_cmd[c_of_s].cmd[0]) \
	 &&!ft_strncmp(my_struct->each_cmd[c_of_s].cmd[0], "cd", ft_strlen(my_struct->each_cmd[c_of_s].cmd[0])))
...
int	somting_in_readline(t_all *my_struct)
{
	...
	if(i == 1)
		return 1;
	c_of_s = 0;
	if(my_struct->number_of_pipes == 1 && if_builtins_in_parent(my_struct, c_of_s))
	{
		if(my_struct->each_cmd[c_of_s].files)
		{
			int output_buckup = dup(STDOUT_FILENO);
			int input_buckup = dup(STDIN_FILENO);
			c_of_s = check_rediractions_in_parent(my_struct, c_of_s);
			if(c_of_s == -1)
			{
			...
			c_of_s = builtins_in_parent(my_struct, c_of_s);
			dup2(output_buckup, STDOUT_FILENO);
			dup2(input_buckup, STDIN_FILENO);
			close(output_buckup);
			close(input_buckup);
