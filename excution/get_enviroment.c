...
/*   Updated: 2023/05/09 15:22:39 by nouakhro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*_get_env(t_list *head, int j, int trim, char *expande_variable)
{
	...
	expande_variable = ft_substr(head->content, j + 1, ft_strlen(head->content
				+ (j + 1)));
...
char	*my_getenv(t_list *head, char *var, int trim)
{
	...
	while (head != NULL)
	{
		...
		{
			j = 0;
			while (++j && (((char *)head->content)[j] == var[j]
				|| ((char *)head->content)[j] == '='))
			{
				if (((char *)head->content)[j] == '=' && !var[j])
				{
					expande_variable = _get_env(head, j, trim, \
						expande_variable);
					return (expande_variable);
				}
			}
