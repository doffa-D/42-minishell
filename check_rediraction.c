...
/*   Updated: 2023/04/17 01:48:06 by nouakhro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void check_rediractions(t_all *my_struct, int c_of_s)
{
    ...
    while(my_struct->each_cmd[c_of_s].files[j].files)
    {
        ...
        if(my_struct->each_cmd[c_of_s].files[j].HERDOC == 1)
        {
            ...
            {
                beffer = readline("> ");
                if(!ft_strncmp(beffer, my_struct->each_cmd[c_of_s].files[j].files, ft_strlen(beffer)))
                    break;
            ...
            close(fd_by_pipe[0]);
            if(my_struct->each_cmd[c_of_s].files[j + 1].files)
