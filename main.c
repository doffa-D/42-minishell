...
/*   Updated: 2023/04/13 17:42:59 by hdagdagu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void cd_commade(t_all my_struct)
{
...
int somting_in_readline(t_all my_struct)
{
    ...
    int j = 0;
    int checker = 0;
    while(my_struct.cmd[i])
    {
        char splite_char = ' ';
        checker = 0;
        while(my_struct.cmd[i] && my_struct.cmd[i] != splite_char)
        {
            if(my_struct.cmd[i] == 34 && !checker)
            {   
                splite_char = 34;
                checker = 1;
            }
            if(my_struct.cmd[i] == 39 && !checker)
            {
                splite_char = 39;
                checker = 1;
            }
        ...
        if(my_struct.cmd[i] == ' ')
        {
            ...
            my_struct.cmd[i] = 3;
            i++;
            my_struct.the_commande = ft_strjoin_v2(my_struct.the_commande, ft_substr(my_struct.cmd , j , i - j));
            j = i;
        }
        if(!my_struct.cmd[i])
        {
