...
/*   Updated: 2023/04/17 17:01:31 by nouakhro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	fix_arg(t_all *my_struct)
{
	...
	while (my_struct->fix_cmd[i])
	{
		...
		{
			if ((my_struct->fix_cmd[i][j] == 34) && my_struct->status != IN_COTE)
			{
				c++;
				if (my_struct->fix_cmd[i][j] == 34 && my_struct->status == IN_DCOTE)
					my_struct->status = OUTSIDE;
				else
					my_struct->status = IN_DCOTE;
				my_struct->the_commande = ft_strjoin_v2(my_struct->the_commande,
						ft_substr(my_struct->fix_cmd[i], c, j - c));
				c = j;
			}
			if ((my_struct->fix_cmd[i][j] == 39) && my_struct->status != IN_DCOTE)
			{
				c++;
				if (my_struct->fix_cmd[i][j] == 39 && my_struct->status == IN_COTE)
					my_struct->status = OUTSIDE;
				else
			...
			if (my_struct->fix_cmd[i][j] == '|' && my_struct->status == OUTSIDE)
			{
			...
			{
				if (my_struct->fix_cmd[i][j + 1] != 39
					&& my_struct->fix_cmd[i][j + 1] != 34)
				{
					if (ft_isalpha(my_struct->fix_cmd[i][j + 1]))
					{
						...
						my_struct->the_commande = ft_strjoin_v2(my_struct->the_commande,
								ft_substr(my_struct->fix_cmd[i], c, j - c));
						while (my_struct->fix_cmd[i][var]
							&& (ft_isalpha(my_struct->fix_cmd[i][var])
								|| ft_isdigit(my_struct->fix_cmd[i][var])))
							var++;
						if (my_struct->status == IN_DCOTE || my_struct->status == OUTSIDE)
						{
							variable = ft_substr(my_struct->fix_cmd[i], j + 1,
									(var - (j + 1)));
							if (getenv(variable))
						...
						{
							variable = ft_substr(my_struct->fix_cmd[i], j, (var
										- j));
							my_struct->the_commande = ft_strjoin_v2(my_struct->the_commande,
									variable);
							j = var - 1;
				...
				else if (my_struct->status == OUTSIDE && (my_struct->fix_cmd[i][j
							+ 1] == 39 || my_struct->fix_cmd[i][j + 1] == 34))
				{
					my_struct->the_commande = ft_strjoin_v2(my_struct->the_commande,
							ft_substr(my_struct->fix_cmd[i], c + 1, j - (c
									+ 1)));
					c = j;
				}
			}
	...
	while (my_struct->splite_pipe[i])
	{
		j = 0;
		my_struct->the_commande = ft_calloc(1, 1);
		if (ft_strchr(my_struct->splite_pipe[i], 2)
			|| ft_strchr(my_struct->splite_pipe[i], 5))
		{
			var = 0;
			while (my_struct->splite_pipe[i][j])
			{
				if (my_struct->splite_pipe[i][j] == 2
					|| my_struct->splite_pipe[i][j] == 5)
				{
			...
			my_struct->each_cmd[i].files = ft_calloc(sizeof(t_files), (var
						+ 1));
		}
		j = 0;
		while (my_struct->splite_pipe[i][j])
		{
			if (my_struct->splite_pipe[i][j] == 2)
			{
				...
				while (my_struct->splite_pipe[i][j]
					&& (my_struct->splite_pipe[i][j] == 2
						|| my_struct->splite_pipe[i][j] == 3
						|| my_struct->splite_pipe[i][j] == 5))
				{
					if (my_struct->splite_pipe[i][j] == 2)
				...
				var = j;
				while (my_struct->splite_pipe[i][var]
					&& my_struct->splite_pipe[i][var] != 2
					&& my_struct->splite_pipe[i][var] != 5
					&& my_struct->splite_pipe[i][var] != 3)
					var++;
				if (my_struct->each_cmd[i].files[c_of_s].number_of_O
					&& my_struct->each_cmd[i].files[c_of_s].number_of_I)
					...
					my_struct->each_cmd[i].files[c_of_s].ERROR_SYNTACSO = 1;
				my_struct->each_cmd[i].files[c_of_s].files = ft_substr(my_struct->splite_pipe[i],
						j, var - j);
				c_of_s++;
				j = var - 1;
			}
					...
					&& my_struct->splite_pipe[i][j] != 2)
			{
				...
				while (my_struct->splite_pipe[i][var]
					...
					var++;
				my_struct->the_commande = ft_strjoin_v2(my_struct->the_commande,
						ft_substr(my_struct->splite_pipe[i], j, var - j));
				j = var - 1;
			}
			j++;
		}
